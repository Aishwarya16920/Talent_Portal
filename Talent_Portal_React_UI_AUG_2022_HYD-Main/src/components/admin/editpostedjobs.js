...
const EditPostedJob = () =>{
    ...
    //const [setUsernameexistError]=useState("");
    //const[IDTitleError,setIDTitleError]=useState("");
    ...
    let error = useSelector((state)=>state.jobs);
    ...
    const editJob=()=>{
                ...
                if(title!=="" && description!=="" && minExp!=="" && maxExp!=="" && primarySkill!=="" && secSkills!==""){
                    ...
                    let newJob = {id,title, description, minExp, maxExp, primarySkill, secSkills,location,authToken}                    
	                let response=dispatcher(editPostedJob(newJob));
                    navigate("/admin");    
...
<br></br>
        ...
        <div class="row justify-content-sm-center h-100">
          <div class="col-md-4 mb-3">
            <br></br>
              <h1 class="fs-4 card-title mb-5" >Update the existing job</h1>
              <form method="PUT" class="needs-validation" novalidate="" autocomplete="off">
              
                  <div class="mb-3">
                      <input id="Job_Title" type="text" placeholder='Job Title' class="form-control" name="Job_Title" required="" ref={jobTitle_ref} onSelect={jobTitle_Error} onBlur={jobTitle_Error} onMouseLeave={jobTitle_Error} onInput={jobTitle_Error} onChange={test}/>
                  </div>
                  ...
                  <div class="row">
                      <div class="col-md-6">
                          <div class="mb-3">
                              <input  type="number" placeholder='Min Experience' class="form-control" name="min" min = "0" max = "50" required="" ref={minExp_ref} onSelect={minExp_Error} onBlur={minExp_Error} onMouseLeave={minExp_Error} onInput= {minExp_Error} onChange={test}/>
                          </div>
                      ...
                      <div class="col-md-6">
                          <div class="mb-3">
                              <input  type="number"  placeholder='Max Experience' class="form-control" name="Max" min= "1" max= "50" required="" ref={maxExp_ref} onSelect={maxExp_Error} onBlur={maxExp_Error} onMouseLeave={maxExp_Error} onInput= {maxExp_Error} onChange={test}/>
                          </div>
                  ...
                  <div class="row">
                      <div class="col-md-6">
                          <div class="mb-3">
                              <input id="Primary_Skill" type="text" placeholder='Primary Skill'  class="form-control" name="Primary_Skill" required="" ref={primarySkill_ref} onSelect={primarySkill_Error} onBlur={primarySkill_Error} onMouseLeave={primarySkill_Error} onInput= {primarySkill_Error} onChange={test}/>
                          </div>
                      ...
                      <div class="col-md-6">
                          <div class="mb-3">
                              <input id="Secondary_Skill" type="text"  placeholder='Secondary Skill' class="form-control" name="Secondary_Skill" required="" ref={secondarySkill_ref} onSelect={secondarySkill_Error} onBlur={secondarySkill_Error} onMouseLeave={secondarySkill_Error} onInput= {secondarySkill_Error} onChange={test}/>
                          </div>
                      ...
                      <div class="form-floating">
                          <textarea class="form-control" placeholder='Description' id="floatingTextarea2" style={{height:"50px"}} required="" ref={description_ref} onSelect={description_Error} onBlur={description_Error} onMouseLeave={description_Error} onInput={description_Error} onChange={test}></textarea>
                        </div>         
                        {descriptionError && <div class="alert alert-danger" role="alert" style={{padding: "5p 15px"}}>{descriptionError}</div>}
                    </div>
                  </div>
                  <div class="align-items-center d-flex" >
                      <Link to = "/admin"><button type="button" onClick = {editJob} class="btn btn-primary ms-auto" disabled={temp} >
                          Update	
                      </button>
                      </Link>
                  </div>
